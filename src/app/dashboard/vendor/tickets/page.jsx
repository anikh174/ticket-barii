...
export default function VendorTickets() {
  ...
  const [editQuantity, setEditQuantity] = useState(""); // সিট সংখ্যা এডিট করার স্টেট
  ...
  // আপডেট বাটন ক্লিক হ্যান্ডলার (মোডাল ওপেন করবে)
  const handleUpdateClick = (ticket) => {
    setSelectedTicket(ticket);
    setEditPrice(ticket.price || "");
    setEditQuantity(ticket.quantity || ""); // ইনিশিয়াল সিট সংখ্যা সেট করা
    
    // datetime-local ইনপুট ফরম্যাটের জন্য YYYY-MM-DDTHH:MM ফরম্যাটে কনভার্ট করা হলো
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/tickets/${ticketId}`, {
        ...
        body: JSON.stringify({
          ...
          quantity: Number(editQuantity), // নতুন সিট সংখ্যা বডিতে পাঠানো হলো
        }),
      });
          ...
          prevTickets.map(t => {
            ...
            if (tId === ticketId) {
              return { 
                ...
                price: Number(editPrice), 
                departureDateTime: editDateTime,
                quantity: Number(editQuantity) // লোকাল স্টেটে আপডেট
              };
            }
            return t;
          })
